...
| File | When | Notes |
|---|---|---|
| `full_setup.sql` | Once, on a new project | Migrations 0001-0006 plus the dev seed, in one transaction. Ends with a verification query. |
...
Both are verified against a real Postgres 16: the setup applies cleanly and
rolls back entirely on failure, and the owner script is idempotent.
